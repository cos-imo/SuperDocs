# Superdocs

## A Propos

SuperDocs est une base de connaissances personnelle portée sur la cybersécurité (mais pas que!) destinée à centraliser, organiser et conserver des ressources en ligne de manière structurée.  
L’objectif est d’offrir une alternative plus durable et exploitable qu’un simple système de favoris de navigateur.

>[!Note]
>SuperDocs est un produit purement personnel, je ne garantis en aucun la fiabilité des liens ni même leur choix

Bonne lecture!


## Sommaire

- 🏠 [Accueil](Accueil.md)

- 🛡️ [Cyber (Blue Team)](cyber%20(blue%20team)/cyber.md)
- 💻 [Dev](dev/dev.md)

  - 📚 [Docs](dev/docs.md)
  - 🌐 [Portals](dev/portals.md)
  - 🧰 [Ressources](dev/ressources.md)
  - 🛠️ [Tools](dev/tools.md)
- ⚙️ [DevOps](devops/devops.md)
- 🔍 [Forensics](forensics/forensics.md)

  - 📚 [Docs](forensics/docs.md)
  - 🛠️ [Tools](forensics/tools.md)
- 🖥️ [Low Level](low%20level/low%20level.md)

  - 💻 [Dev](low%20level/dev.md)
  - 🧩 [Assembly](low%20level/assembly/assembly.md)
    - 🔧 [System](low%20level/assembly/system.md)
  - 🌀 [Reverse](low%20level/reverse/reverse.md)
    - 🌐 [Portals](low%20level/reverse/portals.md)
    - 🧰 [Ressources](low%20level/reverse/ressources.md)
- 🦠 [Malware](malware/malware.md)

  - 🛠️ [Tools](malware/tools.md)
- 🌐 [Networking](networking/networking.md)
- 🥷 [Pentest](🥷%20Pentest.md)

  - 📚 [Docs](📚%20Docs.md)
  - 💾 [Labs](pentest/💾%20Labs.md)
  - 📋 [Lists](Lists.md)
  - 🧰 [Ressources](🧰%20Ressources.md)
	  - 📓 [Playbooks](📓%20Playbooks.md)
	  - 🗺️ [Cheatsheets and mindmaps](src/src/pentest/ressources/Cheatsheets_and_mindmaps.md)
	  - 📋 [Write-ups](src/src/pentest/ressources/write-ups.md)
	  - 🔀 [Misc](src/src/pentest/ressources/Misc.md)
  - 🛠️ [Tools](pentest/tools.md)
